...
let Chaincode = class {
  ...
  // ===============================================
  async write(stub, args, thisClass) {
    if (args.length != 8) {
    ...
    usera.id = id.toString();
    usera.pass=pass.toString();
    usera.name = name.toString();
    usera.phone = phone.toString();
    usera.longi = longi.toString();
    usera.lati = lati.toString();
    usera.time = time.toString();
    usera.balance=balance.toString();

    
    console.log("-------"+id.toString()+" is recived as id of user ");
    ...
    await stub.putState(id.toString(), Buffer.from(JSON.stringify(usera)));
  ...
  async read(stub, args, thisClass) {
    if (args.length != 1) {
      throw new Error('Incorrect number of arguments. Expecting userid of the user to query');
    }
    ...
    let userAsbytes = await stub.getState(id.toString()); //get the user from chaincode state
    if (!userAsbytes.toString()) {
      let jsonResp = {};
      jsonResp.Error = 'user does not exist: ' + id;
  ...
  async transferMarble(stub, args, thisClass) {
    ...
    let userAsbytes = await stub.getState(marbleName);
    if (!userAsbytes || !userAsbytes.toString()) {
      throw new Error('marble does not exist');
    }
    let marbleToTransfer = {};
    try {
      marbleToTransfer = JSON.parse(userAsbytes.toString()); //unmarshal
    } catch (err) {
      let jsonResp = {};
      jsonResp.error = 'Failed to decode JSON of: ' + marbleName;
      throw new Error(jsonResp);
